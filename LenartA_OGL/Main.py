...
class BlockLoader:
    ...
    def init():
        BlockLoader.model = OBJ('./Models/enemy/sat.obj', swapyz=True)
        BlockLoader.modelSizeZ = 3.1  
...
class Block:
    ...
    def draw(self):
        ...
        if self.detectCollisionZ(Player.frontZ, Player.backZ, self.frontZ, self.backZ) \
            ...
            bs=mixer.Sound(os.path.join("assets","explosion.wav"))
            bs.set_volume(0.05) 
            bs.play()
...
class Player:
    ...
    def init():
        Player.model = OBJ('./Models/spaceship/scifi.obj', swapyz=True)
...
class Skybox:
    ...
    def init():
        Skybox.model = OBJ('./Models/space/skybox.obj')
...
class Display:
    ...
    def setCamera():
        glLoadIdentity()
        gluLookAt(0, 4.5, 10,  # ahol a kamera van
                  0, 0, -5,  # ahová a kamera néz
                  0, 1, 0)  
...
class Road:
    ...
    def init(w):
        ...
        imgLoad = pygame.image.load("./Road/track.png")
...
def main():
    ...
    pygame.init()
    pygame.mixer.music.load('./Music/mn.wav')
    pygame.mixer.music.play(-1)
    pygame.mixer.music.set_volume(0.30)
    glutMainLoop()

def get_font(size):
    """Visszaadja a gomb felíratát a kívánt méretben"""
...
def main_menu(): 
    ...
    while True:
        ...
        PLAY_BUTTON = Button(image=pygame.image.load(os.path.join("assets","Play Rect.png")), pos=(400, 250), 
                            text_input="PLAY", font=get_font(50), base_color="#d7fcd4", hovering_color="White")

        QUIT_BUTTON = Button(image=pygame.image.load(os.path.join("assets","Quit Rect.png")), pos=(400, 400), 
